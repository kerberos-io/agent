		...
		if img != nil {
			...
			for cursorError == nil {
				...
				if config.Capture.Continuous == "true" {
					...
					if detectMotion && FindMotion(matArray, coordinatesToCheck, config.Capture.ChangesTreshold) {
...
func FindMotion(matArray [3]*gocv.Mat, coordinatesToCheck [][]int, changesTreshold int) bool {
	...
	if changesTreshold == 0 {
		changesTreshold = 75 //Keep hardcoded value of 75 for now if no value is given for changes treshold in config.json
	}

	return changes > changesTreshold
}
